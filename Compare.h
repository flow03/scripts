	...
	if (!dir_result.empty())
	{
		if (file_1.second != file_2.second)
		{
			...
			if (file_1.second < file_2.second)
			{
				...
				else std::cout << "File " << (dir_result/file_1.first.filename()).string() << " already exists" << std::endl;
			}
			else
			{
				...
				else std::cout << "File " << (dir_result/file_2.first.filename()).string() << " already exists" << std::endl;
			}
		}
...
struct repl_compare : ICompare	// replace
{
	...
	if (!dir_result.empty())
	{
		...
		if (file_1.second != file_2.second)
		{
			...
			{
				difference = (file_1.second - file_2.second)/1024;
				out_str = "Renamed second " + file_2.first.string() + " " + to_string(difference) + " Kb diff";
				rename_path = file_2.first;
				result_path = dir_result/file_2.first.filename();	// operator/ preferred separator
			}
			else
			{
				difference = (file_2.second - file_1.second)/1024;
				out_str = "Renamed first " + file_1.first.string() + " " + to_string(difference) + " Kb diff";
				rename_path = file_1.first;
				result_path = dir_result/file_1.first.filename();
			}
					
			// if ((file_2.second - file_1.second) >= (100 * 1024))
				// std::swap(file_1, file_2);	// need to avoid the second swap
