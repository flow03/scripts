...
class TMX_Merger():
        ...
        for tu in tree.xpath("//tu"):
            if check_prop(tu) is None:
                ...
                if tu_text_pl not in self.tu_dict:
                    ...
                    self.repeat += 1
                    if self.replace_tu(tu, self.tu_dict, tu_text_pl):
                        self.replace += 1
            else: # prop
                ...
                if prop_id not in self.alt_dict:
                    ...
                    self.alt_repeat += 1
                    if self.replace_tu(tu, self.alt_dict, prop_id):
                        self.alt_replace += 1

    def replace_tu(self, tu, tu_dict, key):
        # key = get_pl_text(tu)
        ex_tu = tu_dict[key]
    ...
    def print_stats(self):
        ...
        print("Повторів:", self.repeat)
        print("Замін:\t", self.replace)
        if self.alt_dict:
            print("Альтернативних повторів:", self.alt_repeat)
